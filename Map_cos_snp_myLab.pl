...
open(F4,"/media/LUN4/prajish/exome/processed_data/Recurrence_analysis_exome/GATK_recal_mutect_ori/recurrent_data_with_sample_names.out")||die "Can not open F4.\n";
open(FW,">"."/media/LUN4/prajish/exome/processed_data/Recurrence_analysis_exome/GATK_recal_mutect_ori/recurrent_Map_cos_snp_mylab.txt")||die "Can not open FW.\n";
my (%cosmic, %snp, %mylab)=();
%cosmic=&file(<F1>);
%snp=&file(<F2>);
%mylab=&file(<F3>);
